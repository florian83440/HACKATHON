$(function () {
    ...
    cookie = getCookie("paramBot");
    cookie = transformCookieToArray(cookie);

    if (cookie !== undefined) {
        if (cookie.taille !== undefined) {
            setWebsiteSize(cookie.taille, cookie);
        }
        if (cookie.dalt !== undefined) {

        }
    } else {
        document.cookie = "paramBot=[\"taille\":\"\",\"dalt\":\"\"]";
    }

    function setWebsiteSize(size, cookie) {
        $("body").css("font-size", size / 100 + "em");
        document.cookie = "paramBot=[\"taille\":\"" + size + "\",\"dalt\":\"" + getCookieVal(cookie, "dalt") + "\",\"lecture\":\"" + getCookieVal(cookie, "lecture") + "\"]";
    }

    function setWebsiteDalt(dalt, cookie) {
        if ($("body").hasClass("....")) $("body").removeClass("....");
        if ($("body").hasClass("....")) $("body").removeClass("....");
        if ($("body").hasClass("....")) $("body").removeClass("....");
        $("body").addClass(dalt);
        document.cookie = "paramBot=[\"taille\":\"" + getCookieVal(cookie, "taille") + "\",\"dalt\":\"" + dalt + "\",\"lecture\":\"" + getCookieVal(cookie, "lecture") + "\"]";
    }
    function setWebsiteLecture(lecture, cookie) {
        // si activer, quand on clique sur du text ça le lit
        document.cookie = "paramBot=[\"taille\":\"" + getCookieVal(cookie, "taille") + "\",\"dalt\":\"" + getCookieVal(cookie, "dalt") + "\",\"lecture\":\"" + lecture + "\"]";
    }

    function getCookieVal(data, val) {
        if (data.indexOf(val) !== undefined) {
            return data[val];
        } else {
            return "";
        }
    ...
    function transformCookieToArray(cookie) {
        var result = [];
        datas = cookie.replace("[", "").replace("]", "").replaceAll("\"", "").split(",")
        datas.forEach(data => {
    ...
    function chatbotIA(msg) {
        ...
        if (validAction == true && isAction == 0 && getValidAction != "") {
            if (getValidAction == "taille") {
                setWebsiteSize(getValidData.split("%")[0], cookie);
            } else if (getValidAction == "daltoniens") {
                setWebsiteDalt(getValidData.split("%")[0], cookie);
            } else if (getValidAction == "automaticRead") {
                if (getCookieVal(cookie, "lecture") == "true") {
                    setWebsiteLecture("", cookie);
                } else {
                    setWebsiteLecture("true", cookie);
                }
            }
            ...
            if (error == "") {
                if (action == "taille") {
                    msgIA = "Poucentage : " + data;
                } else if (action == "daltoniens") {
                    msgIA = "Mode Daltoniens";
                } else if (action == "automaticRead") {
                    if (getCookieVal(cookie, "lecture") == "true") { msgIA = "Desactiver le mode lecture : ?"; }
                    else { msgIA = "Activer le mode lecture ?"; }
